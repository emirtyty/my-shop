  ...
  const fetchMyOrders = async () => {
    ...
    try {
      ...
      localStorage.setItem('userPhone', cleanPhone);
        ...
        console.error(err); 
        setUserOrders([]);
    } finally { setIsSearchingOrders(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8F8F8] text-black pb-32">
      ...
      <div className="px-6 flex gap-2 overflow-x-auto no-scrollbar mb-6">
        {categories.map(cat => (
          <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-7 py-2.5 rounded-full text-[10px] font-black uppercase italic tracking-widest transition-all whitespace-nowrap ${activeCategory === cat ? 'bg-orange-500 text-white shadow-lg' : 'bg-white text-zinc-400 border border-zinc-100'}`}>
            {cat}
          </button>
        ))}
      </div>
          ...
          return (
            <div key={p.id} className="bg-white rounded-[2.8rem] p-2 border border-zinc-100 shadow-sm animate-fade-in" style={{ animationDelay: `${index * 0.05}s` }}>
              ...
              <div className="px-3 pb-3 text-center">
                <div className="text-[7px] text-zinc-400 uppercase font-bold mb-1">{p.sellers?.shop_name || 'МАГАЗИН'}</div>
                <h3 className="font-bold text-[10px] uppercase tracking-tighter mb-4 h-8 line-clamp-2 leading-none">{p.name || 'Товар'}</h3>
              ...
              {userOrders.length > 0 ? userOrders.map((order, idx) => (
                <div key={order?.id || idx} className="bg-zinc-50 border border-zinc-100 p-6 rounded-[2.5rem] flex justify-between items-center animate-fade-in shadow-sm">
                  <div className="max-w-[65%]">
                    <div className="text-[11px] font-black uppercase italic leading-tight text-black">{order?.product_name || 'ЗАКАЗ'}</div>
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-[150] bg-black/60 backdrop-blur-md flex items-end animate-fade-in" onClick={() => setIsCartOpen(false)}>
          <div className="bg-white w-full rounded-t-[3.5rem] p-8 animate-slide-up shadow-2xl max-h-[85vh] overflow-y-auto no-scrollbar" onClick={e => e.stopPropagation()}>
                    ...
                    <div key={id} className="flex items-center justify-between bg-zinc-50 p-4 rounded-3xl border border-zinc-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="font-bold text-[10px] uppercase">{item.name || 'Товар'}</span>
            ...
            ) : <p className="text-center py-10 font-black uppercase italic opacity-20 tracking-widest text-[10px]">ПУСТО</p>}
          </div>
        </div>
      )}

      {selectedStory && (
        <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center animate-fade-in" onClick={() => setSelectedStory(null)}>
          <img src={selectedStory} className="max-w-full max-h-[85vh] object-contain" alt="" />
        </div>
      )}
