...
export default function AdminPage() {
  ...
  // ГАРАНТИРОВАННАЯ ФИЛЬТРАЦИЯ ЗАКАЗОВ
  ...
  const updateOrderStatus = async (id: string, newStatus: string) => {
    ...
    if (error) {
      ...
      await fetchData();
      if (newStatus === 'ЗАВЕРШЕН') alert("Успешно: Заказ в архиве");
    }
  };
  ...
  return (
    <div className="p-5 bg-black min-h-screen text-white font-sans pb-20">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-black text-orange-500 uppercase italic">Админ Панель</h1>
      ...
      {showArchivedProducts && (
        <div className="grid gap-3 mb-10 opacity-50">
      ...
      <div className="grid gap-4 mb-10">
        {activeOrders.map(o => (
          <div key={o.id} className="bg-zinc-900 p-5 rounded-[2.5rem] border border-zinc-800 flex justify-between items-center">
             ...
             <select 
               className="bg-black text-[10px] font-black uppercase p-3 rounded-2xl text-orange-500 border border-zinc-800"
               value={o.status}
      ...
      <button onClick={() => setShowFinishedOrders(!showFinishedOrders)} className="w-full py-5 border-2 border-dashed border-zinc-800 rounded-[2.5rem] text-zinc-700 font-black uppercase text-[10px]">
          ...
          {finishedOrders.map(o => (
            <div key={o.id} className="bg-zinc-900/40 p-5 rounded-[2rem] border border-zinc-800 opacity-50 flex justify-between items-center">
              ...
              <div className="flex flex-col items-end">
                 ...
                 <button onClick={() => updateOrderStatus(o.id, 'НОВЫЙ')} className="text-[7px] text-orange-500/50 font-black uppercase underline mt-1">Вернуть</button>
              </div>
            </div>
          ))}
        </div>
